...
# Optionally set the name of the package to install, e.g., for legacy td-agent-bit.
...
curl -fsSL $RELEASE_KEY | gpg --dearmor > /usr/share/keyrings/fluentbit-keyring.gpg
